...
void function DoSuperSpectreDeath( entity npc, var damageInfo )
{
	...
	if ( !npc.IsOnGround() || !npc.IsInterruptable() )
	{
		// just boom
		vector origin = npc.GetWorldSpaceCenter()
...
void function SuperSpectreNukeDamage( int team, vector origin, entity attacker )
{
	...
	for ( int i = 0; i < explosions; i++ )
	{
		...
		RadiusDamage(
			...
			0,													// explosionForce
			DF_EXPLOSION | DF_RAGDOLL | DF_STOPS_TITAN_REGEN,	// scriptDamageFlags
			eDamageSourceId.damagedef_reaper_nuke )				// scriptDamageSourceIdentifier
...
function SuperSpectre_WarpFall( entity ai )
{
    ...
    ai.kv.AccuracyMultiplier = 100.0
