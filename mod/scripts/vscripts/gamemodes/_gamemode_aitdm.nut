...
{
	int teamScoreAddition = abs( GameRules_GetTeamScore( TEAM_MILITIA ) - GameRules_GetTeamScore( TEAM_IMC ) ) / 75 + 2
...
void function AddAITdmPlayerTeamScore( entity player, int score )
{
	...
	AddTeamScore( player.GetTeam(), ScoreAdditionFromTeam( player.GetTeam(), score ) )
	if( player.IsNPC() )
		return
